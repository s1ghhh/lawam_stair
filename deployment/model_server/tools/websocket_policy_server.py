...
class WebsocketPolicyServer:
    ...
    async def run(self):
        logging.info("Binding websocket policy server on ws://%s:%d", self._host, self._port)
        async with websockets.asyncio.server.serve(
            ...
            logging.info("Websocket policy server listening on ws://%s:%d", self._host, self._port)
    ...
    async def _handler(self, websocket: websockets.asyncio.server.ServerConnection):
        ...
        while True:
            try:
                ...
                self._last_active = time.time()
                # Run synchronous policy inference in a worker thread so the
                # websocket event loop can continue answering keepalive pings.
    ...
    def _route_message(self, msg: dict) -> dict:
        ...
        # infer --> framework.predict_action
            ...
            try:
                ...
                # Shared policy instances such as Qwen3VL keep mutable runtime state
                # (for example RoPE cache deltas) and are not safe for concurrent
                # multi-threaded access across websocket clients.
