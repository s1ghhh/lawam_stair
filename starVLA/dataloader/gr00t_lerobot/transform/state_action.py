...
class StateActionTransform(InvertibleModalityTransform):
    ...
    Args:
        ...
        invert_normalized_keys (list[str]): Keys whose normalized semantics should be flipped after
            normalization. `binary` keys use `1 - x`; `min_max` keys use `-x`.
            Intended for aligning gripper semantics across datasets.
    """
    ...
    invert_normalized_keys: list[str] = Field(
        default_factory=list,
        description=(
            "Keys whose normalized semantics should be flipped after normalization. "
            "`binary` keys use `1 - x`; `min_max` keys use `-x`."
        ),
    )
    ...
    def validate_normalization_statistics(self):
        for key in self.invert_normalized_keys:
            if key not in self.apply_to:
                raise ValueError(
                    f"invert_normalized_keys contains '{key}', but it is not present in apply_to"
                )
            if key not in self.normalization_modes:
                raise ValueError(
                    f"invert_normalized_keys contains '{key}', but it has no normalization mode"
                )
            if self.normalization_modes[key] not in {"binary", "min_max"}:
                raise ValueError(
                    f"invert_normalized_keys only supports binary/min_max-normalized keys, got "
                    f"{key}={self.normalization_modes[key]}"
                )
    ...
    @staticmethod
    def _invert_normalized_value(x: torch.Tensor, mode: str) -> torch.Tensor:
        if mode == "binary":
            return torch.ones_like(x) - x
        if mode == "min_max":
            return -x
        raise ValueError(f"Unsupported inversion mode for normalized values: {mode}")
    ...
    def apply(self, data: dict[str, Any]) -> dict[str, Any]:
        for key in self.apply_to:
            ...
            if key in self.invert_normalized_keys and key in self._normalizers:
                state = self._invert_normalized_value(state, self.normalization_modes[key])
            data[key] = state
        return data
