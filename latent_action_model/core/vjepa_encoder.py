...
def _find_local_vjepa_repo() -> Optional[Path]:
    path = Path("/mnt/xx/xx/code/vjepa2")
...
class VJEPAEncoder(nn.Module):
    ...
    def __init__(
        self, 
        model_id: str = "/mnt/xx/xx/weights/vjepa2_1_vitl_dist_vitG_384.pt",
    ...
    def _load_torch_hub_model(self, hub_name: str, checkpoint_path: Optional[str]) -> None:
        ...
        if local_repo is None:
            raise RuntimeError(
                "V-JEPA hub repo not found locally. Please clone the repo into "
                "/mnt/xx/xx/code/vjepa2 (must include hubconf.py)."
            )
